...
def generate_streamlit(string: str) -> str:

    root = etree.fromstring(string)
    elements = root.xpath(
        "//mxCell[string-length(@id) > 1 and string-length(@value) < 40]"
    )
    ...
    def contains(el1: dict, el2: dict) -> bool:
        if el1["x"] <= el2["x"] and el1["y"] <= el2["y"]:
            if (
                el2["x"] + el2["w"] <= el1["x"] + el1["w"]
                and el2["y"] + el2["h"] <= el1["y"] + el1["h"]
            ):
                return True
        return False
    ...
    def recursive_streamlit(el_dict: dict[str, Any], indent: int = 0) -> list[str]:
        ...
        if not len(children):
            if el_dict["class"] == "component":
                ...
                print(
                    "The element below was expecting to have at least one child element."
                )
                return []

        if el_dict["class"] == "window":
            code = [f'{indent * TAB}st.set_page_config(layout="wide")\n']
            children = sorted(children, key=lambda child: (child["y"], child["x"]))
